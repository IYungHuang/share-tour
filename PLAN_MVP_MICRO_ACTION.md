...
- **施工內容**：
  ...
  3. `lib/domain/core_loop/events/curator_event_replay.dart`：`switch (type)` 新增 `case CuratorEventType.difficultySelected:`——只記錄不影響存檔投影數值（局內難度不落局外存檔，比照 `philosophyRerolled` 對 `CuratorSaveData` 無影響的模式，或視需要新增 `lastDifficulty` 投影欄位）。**採極簡版**：`case` 分支留空（no-op），因為難度是單局狀態不是局外累積值，`CuratorSaveData` 不需要它——事件記錄純粹是為了 CC-3 事件溯源的完整性（分析工具日後可從日誌統計難度分佈），不影響任何既有 getter。
- **測試驗證**：
  ...
  - `test/domain/core_loop/events/curator_event_replay_test.dart` 新增：`difficultySelected` 事件不拋例外、`CuratorSaveData` 不受影響（AC-M5-6.3 的姊妹驗證——重播模組本條分支不 import 三態判定模組，靜態檢查沿用既有寫法）。
