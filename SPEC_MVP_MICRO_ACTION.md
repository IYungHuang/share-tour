...
5. **第一層一個字元不動。** 實作上只允許在 `_evaluateHypeInfluencer` 與 `_evaluateBudgetWorker` 的 `return` 前**新增**第二層計算，不得修改 `satisfaction` / `outcome` / `storyBonus` / `baseCommission × commissionRate` 的任何一行。由 AC-M5-9.3 的 golden 分佈比對與靜態 diff 守門。
   ...
   **v6 寫的「`ItineraryStats` 新增 $\sum(\text{hype}_i \times \text{tierFactor}_i)$ 欄位」在 REQ-M5-02.10 的簽章凍結下不可行**（v7 定案，覆核 P0）：`tierFactor` 需要**難度**，而 `calculateStats({philosophy, cameraMultiplier, baseTheme})` 沒有難度參數，同一條又凍結了它的簽章 —— 這個聚合欄位在 `calculateStats` 內部算不出來。**正確分工是兩處各加一塊，缺一不可**：
   ...
   - **`ItineraryStats` 新增一個難度無關的聚合，且須依 `isSpotlight` 再分一維**（v8 修正，覆核 P0）：例如 `hypeSumByTier`（六個整數：`failed`／`normal`／`perfect` × 絕景／非絕景，各自的 $\sum \text{hype}_i$，只看已排入時間線的素材）。
     ...
     `calculateStats` 讀得到每張素材的 `shotTier` 與 `isSpotlight`（皆為既有屬性，不需要難度），故這一步不違反簽章凍結。`spotlightCount` 已存在但語意不同（單純計數，非依 tier 分組的 hype 和），兩者並存不衝突。
...
- **AC-M5-3.7** 中斷（v8 更正，覆核 P0——v7 的測試手段沒有真的觸發 REQ-M5-04.2 的狀態機）：**兩個獨立呼叫皆須送出**——`tester.binding.handleAppLifecycleStateChanged(AppLifecycleState.inactive)`（驅動生命週期，觸發生產程式碼的中斷狀態機，這是 REQ-M5-04.2 定義的觸發條件本身）**加上** `tester.createGesture()` 建立指標、`down(timeStamp: 0)` 後 `gesture.cancel(timeStamp: T)`（供應終止指標事件的 $T_{cancel}$，即判定要用的時戳來源）。驗證判定依 $T - 0$ 比照 REQ-M5-01.3 計算（**非一律 `normal`**）、資源照扣、本局第二層折扣係數變為 0.9；兩次中斷為 0.81。
  > **v7 把「兩者併用」誤寫成「用後者取代前者」，兩件事互不觸發，等於沒測到東西**（v8 更正）。`TestGesture.cancel()` 只送出一個 `PointerCancelEvent`，完全不觸碰 `WidgetsBinding.lifecycleState`；`handleAppLifecycleStateChanged` 也不合成任何指標事件。兩者是彼此獨立的兩條管線——生產程式碼的中斷狀態機掛在生命週期回呼上（REQ-M5-04.2 的觸發條件），指標事件只負責供應時戳。若測試只送 `gesture.cancel()`，狀態機從未進入中斷分支，斷言「本局第二層 × 0.9」會落空；若只送 `handleAppLifecycleStateChanged`，走的是 AC-M5-3.7c 的後備路徑（無終止指標事件），不是本條要驗的正常路徑。**本條唯一正確的形狀是兩者都送**。
  （v6 誤留 v5 舊文「該次判 `normal`」未同步修改，已於 v7 更正；v7 修正時把「補上時戳來源」誤讀成「換掉觸發手段」，v8 修正回「兩者併用」。）
