...
void main() {
  ...
  group('REQ-M5-07.1: expectedPoiId 防護', () {
    test('expectedPoiId 失配時拋出 PoiTargetChangedException，狀態零副作用', () {
      ...
      expect(
        () => controller.gatherPoi('poi_1', expectedPoiId: 'poi_moved_away'),
    ...
    test('QTE 結束時已離開所有 POI 範圍則取消且零副作用', () {
      ...
      expect(
        () => controller.gatherPoi(
          'poi_1',
          manifest: FakeSimpleManifest([attraction]),
          playerPixel: Vector2(500, 500),
          expectedPoiId: 'poi_1',
        ),
        throwsA(isA<PoiTargetChangedException>()),
      );
      ...
      localContainer = ProviderContainer(
        overrides: [
          ...
          poiMaterialResolverProvider.overrideWithValue(
            FakePoiMaterialResolver({
              'poi_1': sampleMaterial,
              'poi_2': otherMaterial,
            }),
          ),
        ],
  ...
  group('難度選定與中斷記錄接線', () {
    ...
    test('recordShutterInterruption 累加中斷次數', () {
      ...
      controller.recordShutterInterruption();
      expect(container.read(curatorRunControllerProvider).interruptionCount, 2);
    });
  });
}
